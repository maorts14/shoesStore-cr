...
def all_shoe(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    return shoes.get_all(db)


@router.get('/{shoe_id}', response_model=schemas.ShowShoe, status_code=status.HTTP_200_OK)
def shoe_by_id(shoe_id, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
...
def delete_shoe(shoe_id, db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
